...
class MovieController {
    ...
    static func getImage(movie: TopLevelDictionary, completion: @escaping (UIImage?) -> Void) {
        let appendURL = posterURL?.appendingPathComponent(String(movie.results[0].image ?? ""))
        print(appendURL)
    }
}
